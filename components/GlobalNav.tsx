    ...
    <nav className="fixed top-0 left-0 right-0 z-50 flex h-20 lg:h-22 xl:h-24 items-center justify-between px-6 lg:px-8 xl:px-12 transition-colors duration-500">
      ...
      <div className="flex items-center gap-2 xl:gap-4">
        ...
        <a 
          ...
          target="_blank" 
          rel="noreferrer"
          className={`group pointer-events-auto flex items-center gap-1 xl:gap-1.5 overflow-hidden rounded-full p-0.5 xl:p-1 pl-3 xl:pl-4 border transition-all duration-300 hidden sm:flex text-[10px] xl:text-[11px] font-bold uppercase tracking-widest ${
            isLight 
              ? "bg-white/10 text-neutral-900 border-neutral-900/10 hover:border-emerald-600 hover:bg-emerald-600/5" 
              : "bg-black/20 text-white border-white/15 hover:border-emerald-500 hover:bg-emerald-500/10"
          }`}
        >
          {/* Sliding Text Area */}
          <span className="relative flex flex-col justify-center items-center overflow-hidden h-5 xl:h-6 min-w-[72px] xl:min-w-[85px]">
            <span className="block w-full text-center transition-transform duration-300 ease-out group-hover:-translate-y-full whitespace-nowrap">
              {t("nav.chat")}
            </span>
            <span className={`absolute inset-0 flex items-center justify-center transition-transform duration-300 ease-out translate-y-full group-hover:translate-y-0 whitespace-nowrap font-bold ${
              isLight ? "text-emerald-600" : "text-emerald-400"
            }`}>
              {language === "bn" ? "চ্যাট করুন" : "Chat Now"}
            </span>
          </span>

          {/* Flying Icon Area */}
          <span className={`relative flex h-6 w-6 xl:h-7 xl:w-7 shrink-0 items-center justify-center overflow-hidden rounded-full transition-all duration-300 ${
            isLight 
              ? "bg-neutral-900/5 group-hover:bg-emerald-600 text-neutral-800 group-hover:text-white" 
              : "bg-white/10 group-hover:bg-emerald-500 text-white"
          }`}>
            <span className="transition-transform duration-500 ease-in-out group-hover:translate-x-[150%] group-hover:-translate-y-[150%] flex items-center justify-center">
              <svg className={`w-3.5 h-3.5 ${isLight ? "text-emerald-600" : "text-emerald-400"}`} viewBox="0 0 24 24" fill="currentColor">
            ...
            <span className="absolute inset-0 flex items-center justify-center text-white transition-transform duration-500 ease-in-out -translate-x-[150%] translate-y-[150%] group-hover:translate-x-0 group-hover:translate-y-0">
