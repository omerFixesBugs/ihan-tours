      ...
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        <div className="text-center mb-16">
          ...
          <h2 
            ...
            dangerouslySetInnerHTML={{ __html: t("contactSection.plan_departure") }}
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          ...
          <div>
            <h3 className="text-xl font-bold text-white mb-6">{t("contactSection.reach_out")}</h3>
            ...
            <div className="space-y-6">
              <div className="flex gap-4">
                ...
                <div>
                  <h4 className="text-white font-medium text-sm mb-1">{t("contactSection.email_us")}</h4>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  <h4 className="text-white font-medium text-sm mb-1">{t("contactSection.office_address")}</h4>
                  <p className="text-neutral-400 font-light">{t("footer.address_val")}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 md:p-10 relative overflow-hidden">
            <h3 className="text-xl font-bold text-white mb-6">{t("contactSection.send_message")}</h3>
            ...
            {formState.status === "success" ? (
              <div className="absolute inset-0 bg-background/95 backdrop-blur-sm flex flex-col items-center justify-center p-8 text-center z-10">
                ...
                <h4 className="text-white text-xl font-bold mb-2">{t("contactSection.msg_sent")}</h4>
                <p className="text-neutral-400 text-sm">{t("contactSection.msg_success_desc")}</p>
              </div>
            ) : null}

            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[11px] uppercase tracking-wider text-neutral-400 mb-2">{t("contactSection.full_name")}</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] uppercase tracking-wider text-neutral-400 mb-2">{t("contactSection.whatsapp_number")}</label>
                ...
                <div>
                  <label className="block text-[11px] uppercase tracking-wider text-neutral-400 mb-2">{t("contactSection.email_address")}</label>
              ...
              <div>
                <label className="block text-[11px] uppercase tracking-wider text-neutral-400 mb-2">{t("contactSection.interested_service")}</label>
                <select className="w-full bg-background border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-brand-red transition-colors appearance-none">
                  ...
                  <option value="tour">{t("contactSection.holiday_tour")}</option>
                  <option value="visa">{t("contactSection.visa_proc")}</option>
                  <option value="ticket">{t("contactSection.air_ticketing")}</option>
                </select>
              </div>

              <div>
                <label className="block text-[11px] uppercase tracking-wider text-neutral-400 mb-2">{t("contactSection.your_message")}</label>
                <textarea required rows={4} className="w-full bg-background border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-brand-red transition-colors resize-none" placeholder={t("contactSection.message_placeholder")}></textarea>
              </div>
